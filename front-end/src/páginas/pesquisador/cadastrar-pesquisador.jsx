...
export default function CadastrarPesquisador() {
  ...
  async function atualizarPesquisador() {
    if (validarCampos()) {
      try {
          const response = await serviçoAtualizarPesquisador({ ...dados, cpf: usuárioLogado.cpf });
        if (response) mostrarToast(referênciaToast, "Pesquisador atualizado com sucesso!", "sucesso");
  ...
  function labelBotãoSalvar() {
    ...
    function açãoBotãoSalvar() {
      if (!usuárioLogado?.cadastrado) cadastrarPesquisador();
