...
import EmpresaFarmacêutica from "../entidades/empresa_farmaceutica";
...
export default class ServiçosUsuário {
  constructor() {}
  static async verificarCpfExistente(request, response) {//alterei
  ...
  static async listarTodosUsuários() {//adicionado
    try {
  ...
  static async verificarCadastroCompleto(usuário: Usuário) {
    switch (usuário.perfil) {
      case Perfil.PESQUISADOR:
        ...
        return true;
      case Perfil.EMPRESAFARMACEUTICA:
        const empresa_farmaceutica = await EmpresaFarmacêutica.findOne({
          where: { usuário: usuário.cpf },
          relations: ["usuário"],
        });
        if (!empresa_farmaceutica) return false;
        return true;
      default:
        return;
    }
