...
const SALT = 10;
const SENHA_JWT = process.env.SENHA_JWT || 'MinhaChaveSecreta';
  ...
  static async verificarCadastroCompleto(usuário: Usuário) {
    switch (usuário.perfil) {
      case Perfil.PESQUISADOR:
        ...
        return true;
      case Perfil.GERENTEEMPRESAFARMACEUTICA:
        const gerente_empresa_farmaceutica = await GerenteEmpresaFarmacêutica.findOne({
          where: { usuário: usuário.cpf },
          relations: ["usuário"],
        });
        if (!gerente_empresa_farmaceutica) return false;
        return true;
      default:
        return;
    }
