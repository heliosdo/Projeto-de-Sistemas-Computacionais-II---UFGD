...
export default class ServiçosPesquisador {
  ...
  static async cadastrarPatente(request, response) {
    try {
      const { numero, categoria, ano_concessao, paises_abrangidos, data_inicio, exclusividade_mercado,
        cpf } = request.body;
      const cpf_encriptado = md5(cpf);
      const pesquisador = await Pesquisador.findOne({
        where: { usuário: cpf_encriptado },
      ...
      await Patente.create({
        numero, categoria, ano_concessao, paises_abrangidos, data_inicio, exclusividade_mercado,
        pesquisador
      }).save();
      return response.json();
    } catch (error) {
  ...
  static async alterarPatente(request, response) {
    try {
      const { id, numero, categoria, ano_concessao, paises_abrangidos, data_inicio, exclusividade_mercado } = request.body;
      await Patente.update(id, { numero, categoria, ano_concessao, paises_abrangidos, data_inicio, exclusividade_mercado });
      return response.json();
    } catch (error) {
  ...
  static async buscarInteressesPatente(request, response) {
    try {
      const id_patente = request.params.id_patente;
      const interesses = await Interesse.find({
        where: { patente: { id: id_patente } },
        relations: ["gerente_empresa_farmaceutica", "gerente_empresa_farmaceutica.usuário", "patente"]
      });
      return response.json(interesses);
    } catch (error) {
      return response.status(500).json(
        { erro: "Erro BD : buscarInteressesPatente" });
    }
  };
